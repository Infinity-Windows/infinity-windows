...
export function ModelStudio({ source }: { source: StudioSource }) {
   ...
   * scale, and re-cut its wall hole. Thin Studio-only wrapper over the
   * shared `applyUnitGeometry` core (lib/modelstudio/unitGeometry.ts):
   * Studio is the one editable scene, so it's the one caller that honors a
   * per-item gap override, rebuilds moving-sash rigs, and repaints the
   * glyph/label overlay — every other viewer takes the core's defaults. */
  const applyUnitGeometry = (item: StudioItem, config: UnitConfig, frameGapMm?: number) => {
    applyUnitGeometryCore(item, config, {
      frameGapMm,
      allowStoredFrameGap: true,
      rebuildMovers: true,
      onApplied: (it) => setUnitAnnotations(it, selUnitRef.current === it),
    });
  };
