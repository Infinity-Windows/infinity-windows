...
import {visibleUpdates,receiptIds,safeUpdateLink,updateRoleRank,readUpdateReceipt,markUpdatesRead,type AppUpdate,updateBuildReceipt} from './appUpdates';
...
describe('role-specific app improvements',()=>{
 it('acknowledges one installed build without suppressing a later release',()=>{markUpdatesRead('build-reader',[updateBuildReceipt('release-a')]);const ids=receiptIds(readUpdateReceipt('build-reader'));expect(ids).toContain(updateBuildReceipt('release-a'));expect(ids).not.toContain(updateBuildReceipt('release-b'));});
