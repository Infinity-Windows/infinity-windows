...
export function mountFitView(host, job, shim) {
  ...
  var state = {
    ...
    sel: null, labels: "full", view: "plan", q: "", storyFocus: 0,
  ...
  var HINT_DEFAULT = "Drag to orbit &middot; pinch or scroll to zoom &middot; tap a unit for its spec";
  ...
  function exitAssign() {
    ...
    $("hint").innerHTML = "Drag to orbit &middot; pinch or scroll to zoom &middot; tap a unit for its spec";
  }
  ...
  // stories: one tap answers "show me the story-3 windows" on a tall
  // building. The active story's walls stay lit; every other story drops to
  // a dim x-ray, and its wall chips hide so the strip only offers walls you
  // can actually read.
  function setStoryFocus(n) {
    state.storyFocus = n;
    Array.prototype.forEach.call(ROOT.querySelectorAll(".face"), function (f) {
      f.classList.toggle("story-dim", !!n && f.dataset.story !== String(n));
    });
    Array.prototype.forEach.call(ROOT.querySelectorAll("#storyStrip .elev"), function (c) {
      c.setAttribute("aria-pressed", String(c.dataset.story === String(n || "all")));
    });
    buildElevStrip();
    syncElevChips();
  }
  ...
  function buildStoryStrip() {
    var strip = $("storyStrip");
    strip.innerHTML = "";
    strip.hidden = STORIES.length < 2;
    if (STORIES.length < 2) return;
    var all = document.createElement("button");
    all.className = "elev";
    all.dataset.story = "all";
    all.setAttribute("aria-pressed", String(!state.storyFocus));
    all.textContent = "All floors";
    all.addEventListener("click", function () { setStoryFocus(0); });
    strip.appendChild(all);
    STORIES.forEach(function (st) {
      var items = JOB.windows.filter(function (w) { return (w.story || 1) === st.n; });
      var open = items.filter(function (w) { return w.status !== "installed"; }).length;
      var b = document.createElement("button");
      b.className = "elev";
      b.dataset.story = String(st.n);
      b.setAttribute("aria-pressed", String(state.storyFocus === st.n));
      b.innerHTML = esc(st.name) + " <b>" + open + "/" + items.length + "</b>";
      b.addEventListener("click", function () {
        setStoryFocus(state.storyFocus === st.n ? 0 : st.n);
      });
      strip.appendChild(b);
    });
  }
  ...
  function buildElevStrip() {
    ...
    ELEVS.forEach(function (e) {
      // stories: with a story focused, other stories' walls leave the strip.
      if (state.storyFocus && (e.story || 1) !== state.storyFocus) return;
