...
/** Every accessible crew punch, with an optional clock-in date window.
 * Exact counts and unique IDs prevent a server cap or moving pages from being
 * presented as a complete payroll/job total. Null bounds mean all saved time.
 */
export async function listTeamShifts(
  ...
  for (let page = 0; page < 1000; page++) {
    let query = supabase.from("time_shifts")
    ...
    if (typeof count !== "number" || (expected !== null && count !== expected)) {
      throw new Error("Time records changed or could not be counted. Refresh the report.");
    }
    expected = count;
    const rows = (data ?? []) as TimeShift[];
    if (!rows.length && out.length < expected) {
      throw new Error("The time report is incomplete. Please refresh.");
    }
    out.push(...rows);
    if (out.length >= expected) {
      if (out.length !== expected || new Set(out.map((row) => row.id)).size !== expected) {
