...
for(const width of [390,1440])test(`hours report and snapshot downloads at ${width}px`,async({page})=>{
 ...
 let requests=0;
 await page.route('**/functions/v1/ask',async route=>{requests++;const body=route.request().postDataJSON();expect(body.timeZone).toBeTruthy();await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({answer:'**Recorded time:** **8 hours** after breaks.\n- **Job:** Richardson Brothers_Estates at Sand Hollow 18.\n- The report includes unassigned time.',sources:[],artifacts:[report]})});});
 ...
 await expect(page.locator('.ask-bubble').last()).toContainText('Recorded time: 8 hours');await expect(page.locator('.ask-bubble').last()).not.toContainText('**');await expect(page.locator('.ask-bubble').last()).toContainText('Richardson Brothers_Estates at Sand Hollow 18');
