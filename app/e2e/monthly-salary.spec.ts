...
async function setup(page:Page,role:"owner"|"foreman"|"supervisor"="owner",payGrant=false) {
...
test("supervisor without the pay grant sees no salary or pay controls",async({page})=>{
 await setup(page,"supervisor");await expect(page.getByRole("heading",{name:"Roster",exact:true,level:1})).toBeVisible();
...
test("a pay reader can review but cannot change compensation",async({page})=>{
 const {panel}=await setup(page,"supervisor",true);await expect(panel).toBeVisible();
...
for (const role of ["installer", "foreman"] as const) {
 test(`${role} cannot open Roster even with a pay grant`, async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 900 });
  await installFixtures(page, { role });
  await hideWrongProjectBanner(page);
  await page.route("**/rest/v1/profiles**", r => { const p = {
   id: TEST_USER.id, display_name: "Fixture user", role, active: true,
   skill_level: 3, language: "en", can_see_pay: true, can_see_costs: true,
  }; return json(r, new URL(r.request().url()).searchParams.has("id") ? p : [p], 1); });
  await page.goto("/crew");
  await expect(page.getByRole("heading", { name: "Not available for your role" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Roster", exact: true })).toHaveCount(0);
  await expect(page.locator('a[href="/crew"]')).toHaveCount(0);
  if (role === "foreman") {
   await page.goto("/team");
   await expect(page.getByRole("heading", { name: "People hub" })).toBeVisible();
   await expect(page.locator('a[href="/crew"]')).toHaveCount(0);
  }
 });
}
