...
-- a signature that could not be sent shows. The roster's group sign-in still
-- needs signal — that is a supervisor-only screen, so it is not in a note
-- every installer reads (the screen says it itself). The quoted words are the
-- phone's own:
