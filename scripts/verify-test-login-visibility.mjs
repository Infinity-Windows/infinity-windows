...
// before and after 20261030030000 (a test login sees a job that is both a
// testing project and on the sandbox list). No network, no production
// database: the real migrations over stubs of the platform (auth, profiles,
// the sandbox list, partner grants, the role helpers).
...
create table profiles(id uuid primary key,role text not null,active boolean default true,is_partner boolean not null default false,is_test boolean not null default false);
...
// People. The two QA logins are the only profiles flagged is_test.
const PEOPLE = {
...
await db.query("update profiles set is_partner=true where id=$1", [PEOPLE.partner]);
await db.query("update profiles set is_test=true where id=any($1)", [[PEOPLE["qa.installer"], PEOPLE["qa.foreman"]]]);
...
async function seen(uuid) {
  await as(uuid);
  const rows = (await db.query("select id from projects order by job_code")).rows;
  await db.exec("reset role");
  return rows.map((r) => codeOf[r.id]).sort();
}
...
for (const who of ["installer", "foreman", "supervisor", "owner", "partner"]) {
