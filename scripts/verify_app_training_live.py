...
"""Judge who can open the "Using Forge" walkthrough videos, on the live database.
    ...
    storage files (bucket app-training) it can read. This module turns those
    answers into a pass/fail and the markdown table the workflow prints —
    the same split as scripts/verify_invariants.py: the shell script asks,
    this judges.

Exit 0 when every checked person saw exactly what the role floor says they
should (installer 0+, foreman 1+, leadership 2+ — docs/role-training-videos.md,
"Who can watch"), 1 when one of them saw something else — or when nothing
could be measured. A check that cannot tell "clean" from "did not look" is
worse than no check, so an answer that did not run as the simulated login,
and a run that checked nobody at all, both FAIL rather than pass quietly.

Only role names, head counts and video slugs are printed here — never an id
or a name.
...
def judge(published: list[dict], answers: list[dict]) -> tuple[dict, int]:
    ...
    many were wrong, and what that role should see — and the total count of
    people who saw something other than what their role allows.
...
def render_table(published: list[dict], by_role: dict) -> str:
    ...
    lines = [
        ...
        "Published now: "
        + (", ".join(f"{p['slug']} ({p['min_role']}+)" for p in published) or "nothing")
        + "\n",
    ...
    for role in sorted(by_role, key=lambda k: (ROLE_ORDER.index(k) if k in ROLE_ORDER else 99, k)):
        ...
        lines.append(f"| {role} | {g['n']} | {_fmt(g['should'])} | {result} |")
    return "\n".join(lines)
