-- PR #639: its crew note (docs/app-updates.md). Read it as the system to prove
...
begin
  ...
  select count(*) into v_n from public.app_release_notes
   where id = '2026-09-25-unit-photos-send'
     ...
     and published_on = date '2026-09-25' and withdrawn_at is null and href = '/stuck'
  ...
  perform pg_temp.dry_run_check('the note is stored as written', v_n = 1,
    format('expected 1 row for all four roles linking to /stuck, got %s', v_n));
