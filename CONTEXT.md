...
**Bay** — a job's own box (kind `bay`), one per job, made with the job (ADR-0009, 2026-09-06). **Set aside** stores a package into its job's bay like any other box, so material waiting for a job is findable on the yard. The old rack-and-slot shelves held nothing and are retired; there is one location model. On the warehouse page the bays are their own view beside the boxes (owner call 2026-09-06), and a bay can be **turned off** once its job's material has gone out — it must be empty first, and the job gets a fresh bay on its own the next time something is set aside for it.
...
**On-behalf punch** — a shift a supervisor started or ended *for* somebody, from the Team timecards roster, rather than one the person tapped themselves. It is a real punch and pays exactly like any other; what it carries is the name of whoever pressed it (`clocked_in_by` / `clocked_out_by` — null still, and forever, means "they did it"). Because it moves a person's pay without their hand on the phone, it is never silent: every one also writes a `time_shift_edits` row, so it lands in the audit trail supervisors read *and* in the worker's own "your timecard was changed" feed, and the running punch says "Clocked in by Marlene" on their own clock until it ends. Supervisor+ only — the same tier that can already edit and void time, because starting somebody's punch *is* an edit to their pay. Deliberately carries no location: the phone that pressed it is standing in the shop, and stamping fourteen crew punches with the office's coordinates would put fourteen people somewhere none of them chose to report.

**Group sign-in** — today's toolbox talk recorded for a person by the supervisor who gave it to them in the yard, instead of by their own signature on their own phone. It satisfies the clock-in gate exactly as a signature does, which is the point: a bulk clock-in that skipped the gate would put a crew on the clock with nobody having heard a safety talk. It is stored as the *weaker* record it is — `signed_via = 'group'`, `signed_by` = the supervisor, and no typed name, no drawn signature, no archived PDF — so a compliance read can always tell an attestation from a signature. It is never a duplicate and never an overwrite: somebody who already signed for themselves keeps their own row.
...
**Tap time / arrival time** — the two times every clock action carries since Release 0 (K0.5, owner decision 2026-09-23, `20261028000000`): when the phone's button was pressed, and when the request reached the server. Pay uses the tap time when the phone's clock was checked against the server within the last 24 hours and found within 2 minutes, and the tap (corrected by the measured skew) precedes arrival, is within the shift cap, is not before the shift's last punch (its clock-in, the break it is ending, or a break it already finished — `time_shifts.last_punch_at`, stamped by every clock RPC because `break_started_at` is cleared when a break ends) and, for a clock-in, is not before the end of the person's previous shift (a completed or approved one counts; a voided one has left the timeline); otherwise pay uses the arrival time — every punch's only time before this — and the punch is marked **time needs review**. A punch that sends no tap time at all is an old bundle, not a suspect: arrival time, no mark. The chosen time lives in the ordinary `clock_in_at` / `clock_out_at` / `break_seconds` columns so nothing downstream changes shape; both raw times, the check and the decision live in the `time_clock_actions` ledger, which is also the memory that makes a punch one-time (K0.2): every clock in, clock out, break start and break end carries a one-time id, a repeat of the id is answered with the original result, and `clock_out` refuses anything but an open shift, so a resend can never move a time or re-submit an approved week.

**Time needs review** — `time_shifts.review_reason`, a short code the timecard translates, set by the clock itself and never cleared by it: the tap time could not be trusted (which rule failed is the code), the previous shift was still open, or a break end arrived with no break running (K0.4 — the lunch return that could not find its lunch start is refused out loud, kept on /stuck, and marked here, with an audit line in `time_shift_edits`). Distinct from `time_confirmed = false`, which is the crew member's own "my time is wrong" at clock-out.
...
**Removed** — the word for a login taken away for good, settled 2026-09-04, and the third of three things that can be true about a person. `active` is availability ("on site today", a foreman toggles it every morning). `access_revoked_at` is a switched-off login, and it is reversible — the auth user is banned and "Let them back in" un-bans them. `retired_at` is Removed: banned AND its email handed back, so there is nothing to switch back on and the way back is a fresh invite. Owner-only, and it picks between two shapes by COUNTING rather than asking (`person_record_counts`): a login with zero rows in any work, money or safety table is deleted outright — `profiles.id` references `auth.users` ON DELETE CASCADE, so nothing survives it because there was nothing to survive — and a login with anything at all keeps every row, under the same profile id and the same display name. The name is never rewritten; the Roster greys the row and says Removed beside it. The point of the whole door is freeing the ADDRESS: Supabase Auth holds emails unique forever, so a banned account keeps its address hostage and the same person could never be given a fresh account. Renaming it to `<uid>@removed.invalid` frees it without touching one row of history.
