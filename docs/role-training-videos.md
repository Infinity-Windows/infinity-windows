...
- Every night, and on every pull request, the same rule is proven again on the **live** database, not a copy: `scripts/verify-app-training-live.sh` signs in as every real, non-removed account, one at a time inside a read-only transaction, and asks which catalog rows — identified by `slug/language/version`, so a published bilingual pair never reads as a false alarm — and which exact private storage object paths that account can actually open. `scripts/verify_app_training_live.py` does the judging and is covered offline, with no token or network, by `scripts/test_verify_app_training_live.py`. It fails if any account can see a catalog row or a storage object its role should not, if a video, captions or poster path a visible row promises turns out missing from storage, if a check somehow did not run as that account's own login, or if nobody got checked at all. It proves who can open which rows and files, and that each expected file exists in storage — it does not play the video, and it does not test signed-link behavior or real Safari playback.
