...
- Every night, and on every pull request, the same rule is proven again on the **live** database, not a copy: `scripts/verify-app-training-live.sh` signs in as every real, non-removed account, one at a time inside a read-only transaction, and asks which catalog rows and which private storage files that account can actually open. `scripts/verify_app_training_live.py` does the judging and is covered offline, with no token or network, by `scripts/test_verify_app_training_live.py`. It fails if any account can see a walkthrough its role should not, if a check somehow did not run as that account's own login, or if nobody got checked at all.
